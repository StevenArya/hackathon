...
export function ActivityFeed({ customers, invoices, today, expanded = false, unavailable }: { customers: Customer[]; invoices: Invoice[]; today: string; expanded?: boolean; unavailable?: boolean }) {
  ...
  return <div>{!items.length && <p className="px-6 py-14 text-center text-sm text-stone-500">{unavailable ? "Activity is unavailable while disconnected." : "No invoice or customer activity to show yet."}</p>}{(expanded ? items : items.slice(0, 5)).map(item => <div key={item.id} className="activity-item"><span className={`activity-icon tone-${item.tone}`}><Icon name={item.icon} className="!h-4 !w-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-medium leading-5">{item.title}</p><p className="mt-1 text-[11px] text-stone-500">{item.description}</p></div><span className={`status-badge tone-${item.tone}`}>{item.tag}</span></div>)}</div>;
}
