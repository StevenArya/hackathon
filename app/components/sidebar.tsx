...
export function Sidebar({ active, onNavigate, connected }: { active: Section; onNavigate: (section: Section) => void; connected: boolean }) {
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      <button type="button" onClick={() => { onNavigate("Dashboard"); setOpen(false); }} aria-label="InVora dashboard" className="flex items-center gap-2.5 px-3 text-xl font-semibold tracking-tight"><span className="brand-mark"><span /><span /><span /></span>InVora<span className="text-pink-700 text-[1.05em] font-semibold">.</span></button>
      <div className="workspace"><span className="workspace-avatar text-[1.05em] font-semibold">CI</span><div className="flex-1"><p className="text-xs font-semibold">Credit Intelligence</p><p className="mt-1 text-[11px] text-stone-500">Business workspace</p></div><Icon name="chevron" className="!h-3.5 !w-3.5 text-stone-500" /></div>
      <p className="nav-label">WORKSPACE</p>
      <nav aria-label="Main navigation" className="space-y-1">{sections.map((section, index) => <button key={section} onClick={() => { onNavigate(section); setOpen(false); }} aria-current={active === section ? "page" : undefined} className={`nav-item ${active === section ? "nav-active" : ""}`}><Icon name={icons[index]} />{section}{section === "AI Assistant" && <span className="ml-auto rounded border border-pink-100 px-1.5 py-0.5 text-sm text-pink-700 font-semibold">BETA</span>}</button>)}<button type="button" className="nav-item" onClick={() => { setLogoutError(""); logoutDialog.current?.showModal(); }}>Logout</button></nav>
